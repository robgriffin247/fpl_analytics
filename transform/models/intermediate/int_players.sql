...
add_appearances as (
  select
    *,
    case when isnan(total_points / points_per_appearance) then 0 else (total_points / points_per_appearance)::int end as appearances,
  from variable_fixes
),
...
add_fixtures as (
  ...
  from decode_position 
    left join team_fixtures on decode_position.team=team_fixtures.team_abbreviation and decode_position.gameweek=team_fixtures.gameweek
),
